			...
			for (int i = 0; i < 4 - !multiband * 2; ++i) {
				...
				switch(distType[i]) {
					...
					case 4: { // sinusodal
						...
						// x - x^3 / 6 + x^5 / 120
						float modInput = std::fmod(distIn - numbers::pi_v<float> * 0.5f, 2.f * numbers::pi_v<float>);
						if (modInput < 0) {modInput += 2.f * numbers::pi_v<float>;}
						const float x = std::abs(modInput - numbers::pi_v<float>) - numbers::pi_v<float> * 0.5f;
						const float x2 = x * x;
						const float x3 = x2 * x;
...
void SlewDistortion::changeSampleRate()
{
	...
	m_dcCoeff = std::exp(-2.f * numbers::pi_v<float> * SLEW_DISTORTION_DC_FREQ / sampleRateOver);
